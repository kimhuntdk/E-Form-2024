<?php 
	...
	$Save_Staff_withdraw_taking_leave_registration = trim($_POST['Save_Staff_withdraw_taking_leave_registration']); 
	...
	if($Save_Staff_withdraw_taking_leave_registration=="Save_Staff_withdraw_taking_leave_registration"){ // Staff Save
		...
		if($staff_grad_approve_disapprove==3){ //กรณีมีแก้ไข ส่งคืนนิสิตให้เก็บลงอีก table request_repatriate
			...
			$mysqli->query( $sql_in );
		// ไปอัฟเดพ 
		$sql = "Update request_withdraw_taking_leave_registration set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_date=Now() Where doc_id='$doc_id' ";
	...
	"; //รายละเอียดที่ส่ง
		...
		$sql = "Update request_withdraw_taking_leave_registration set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_node='$argument_a',staff_grad_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
			if($rs_doc){
				$mail->Send();
			echo 1; //เพิ่มข้อมูลสำเร็จ
