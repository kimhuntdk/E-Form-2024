...
$sql = "SELECT * FROM request_doc LEFT JOIN request_student_status_retention ON request_doc.doc_id=request_student_status_retention.doc_id WHERE request_doc.doc_id=" . $doc_id;
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<h3 class="card-title">คำร้องขอลงทะเบียนรักษาสภาพนิสิต (ระดับบัณฑิตศึกษา) Student Status Retention Form  (Graduate Student)</h3>
								</div>
								<div class="card-body">
									...
									<form action="javascipt:;" method="post" id="frm_add_status_retention" class="frm_add_status_retention">
										<div class="form-group">
											<div class="row align-items-center">
												<label class="col-sm-12">มีความประสงค์<span class="card-title">ขอลงทะเบียนรักษาสภาพนิสิต</span>/ Would like to <span class="card-title">Student Status Retention</span> in <span class="form-required">*</span> </label>
											</div>
										</div>
                  ...
                  <span class="custom-control-label">แบบฟอร์ไม่ถูกต้อง</span> </label>
                          ...
                          <option value="5" <?php if($row[dean_admin] ==5) { echo "selected";}?>>รองคณบดีบัณฑิตวิทยาลัยฝ่ายประกันคุณภาพการศึกษาและกิจการพิเศษ</option>
										...
										<div class="btn-list mt-4 text-left">
											...
											<input type="hidden" name="Save_Staff_Status_Retention" value="Save_Staff_Status_Retention">
		...
		$( "#btnSaveSign" ).click( function () { //ส่งค่าไป Update คำร้อง
			...
			if ( staff_grad_approve_disapprove === "1" ) {
				...
				if ( dean_admin === "0" ) {
					...
					$.post(
						"save_staff_status_retention.php",
						$( "#frm_add_status_retention" ).serializeArray(),
						function ( data ) {
							//alert(data);
							if ( data === "1" ) {
