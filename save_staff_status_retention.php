<?php 
	...
	$Save_Staff_Status_Retention = trim($_POST['Save_Staff_Status_Retention']); 
	...
	// สิ้นสุด				//$mail->AddAddress('$advisor_email','$advisor_name_full'); //อีเมล์และชื่อผู้รับ
				
	
	if($Save_Staff_Status_Retention=="Save_Staff_Status_Retention"){ // Staff Save
		...
		if($staff_grad_approve_disapprove==3){ //กรณีมีแก้ไข ส่งคืนนิสิตให้เก็บลงอีก table request_repatriate
			...
			$mysqli->query( $sql_in );
		// ไปอัฟเดพ 
		$sql = "Update request_student_status_retention set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_date=Now() Where doc_id='$doc_id' ";
	...
	"; //รายละเอียดที่ส่ง
		...
		$sql = "Update request_student_status_retention set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_node='$argument_a',staff_grad_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
			if($rs_doc){
				$mail->Send();
			echo 1; //เพิ่มข้อมูลสำเร็จ
	...
	กรณีพบปัญหา/ข้อสงสัย ติดต่อที่ graduate@msu.ac.th
	"; //รายละเอียดที่ส่ง
				$mail->AddAddress($dean_mail,$dean_name); //อีเมล์และชื่อผู้รับ$mail->Send();
			echo 1; //เพิ่มข้อมูลสำเร็จ
		} else {
			echo 2; //ไม่สามารถเพิ่มข้อมูลได้
		}
	...
	elseif($Save_admin_Leave=="Save_admin_Leave"){ // Admin Save
		$sql = "Update request_student_status_retention set dean_admin_approve_disapprove='$admin_grad_approve_disapprove',dean_admin_node='$argument_a',dean_admin_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
		if($rs_doc){
			echo 1; //เพิ่มข้อมูลสำเร็จ
		} else {
