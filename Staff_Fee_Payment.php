...
$sql = "SELECT * FROM request_doc LEFT JOIN request_fee_payment ON request_doc.doc_id=request_fee_payment.doc_id WHERE request_doc.doc_id=" . $doc_id;
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<h3 class="card-title">คำร้องชำระค่าธรรมเนียมการศึกษา /Request Form for Fee Payment</h3>
								</div>
								<div class="card-body">
									...
									<form action="javascipt:;" method="post" id="frm_add_fee_payment" class="frm_add_fee_payment">
										<div class="form-group">
											<div class="row align-items-center">
												<label class="col-sm-12">มีความประสงค์ขอชำระค่าธรรมเนียมการศึกษา/ Would like to take a leave in <span class="form-required">*</span> </label>
											</div>
										</div>
                                           ...
                                           <div class="form-group">
											<div class="row align-items-center">
												...
												<div class="form-group col-sm-9">
											     <div class="custom-controls-stacked">
													<label class="custom-control custom-radio custom-control-inline">
                <input type="radio" class="custom-control-input"  value="1" name="Because" id="Because" <?php if($row[ 'because_status']==1 ) { echo "checked"; } ?>>เนื่องจาก/ because <span class="form-required">*</span> <span class="custom-control-label">เรียนครบหลักสูตรแล้ว ต้องการชำระค่าธรรมเนียมการศึกษาที่เหลือตามหลักสูตรกำหนด เพื่อสำเร็จการศึกษา(I have completed all required course, but I am not graduated yet</span>
                </label>
												</div>
                                                </div>
											</div>
                                 		...
                                 		<div class="form-group">
											<div class="row align-items-center">
												...
												<div class="form-group col-sm-9">
											     <div class="custom-controls-stacked">
													<label class="custom-control custom-radio custom-control-inline">
                <input type="radio" class="custom-control-input" name="Because" id="Because" value="2" <?php if($row[ 'because_status']==2 ) { echo "checked"; } ?> ><span class="custom-control-label">อื่น ๆ (other) </span><input type="text" class="form-control" name="other" id="other" value="<?php echo $row[ 'because']; ?>">
                </label>
												</div>
                                                </div>
											</div>
                                        ...
                                        <div class="form-group">
											<div class="row align-items-center">
                                        *ให้นิสิตดำเนินการให้แล้วเสร็จภายใน 2 สัปดาห์แรกนับจากวันเปิดภาคเรียนปกติ หรือภายในสัปดาห์แรก นับจากวันเปิดภาคการศึกษาพิเศษมิฉะนั้นต้องเสียค่าปรับตามอัตราที่มหาวิทยาลัยกำหนด (Students must complete the process within the first 2 weeks starting from the beginning of the first and second semester or within the first week of the 3rd semester. If not, students have to pay for a fine regarding the university rate)
										</div>
                                        </div>
                  ...
                  <span class="custom-control-label">แบบฟอร์ไม่ถูกต้อง</span> </label>
                          ...
                          <option value="5" <?php if($row[dean_admin] ==5) { echo "selected";}?>>รองคณบดีบัณฑิตวิทยาลัยฝ่ายประกันคุณภาพการศึกษาและกิจการพิเศษ</option>
										...
										<div class="btn-list mt-4 text-left">
											...
											<input type="hidden" name="Save_Staff_fee_payment" value="Save_Staff_fee_payment">
		...
		$( "#btnSaveSign" ).click( function () { //ส่งค่าไป Update คำร้อง
			...
			if ( staff_grad_approve_disapprove === "1" ) {
				...
				if ( dean_admin === "0" ) {
					...
					$.post(
						"save_staff_fee_payment.php",
						$( "#frm_add_fee_payment" ).serializeArray(),
						function ( data ) {
							//alert(data);
							if ( data === "1" ) {
