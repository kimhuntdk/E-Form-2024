<?php 
	...
	$Save_Staff_change_study_program = trim($_POST['Save_Staff_change_study_program']); 
	...
	if($Save_Staff_change_study_program=="Save_Staff_change_study_program"){ // Staff Save
		...
		if($staff_grad_approve_disapprove==3){ //กรณีมีแก้ไข ส่งคืนนิสิตให้เก็บลงอีก table request_repatriate
			...
			$mysqli->query( $sql_in );
		// ไปอัฟเดพ 
	echo	$sql = "Update request_change_field_study set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_date=Now() Where doc_id='$doc_id' ";
	...
	"; //รายละเอียดที่ส่ง
				...
				$mail->AddAddress("kim_4692@hotmail.com",$dean_name); //อีเมล์และชื่อผู้รับ
				$mail->Send();
	...
	echo	$sql = "Update request_change_field_study set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_node='$argument_a',staff_grad_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
			if($rs_doc){
				$mail->Send();
			echo 1; //เพิ่มข้อมูลสำเร็จ
		...
		$sql = "Update request_change_field_study set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_node='$argument_a',staff_grad_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
		if($rs_doc){
			 $mail_test = "jakkrid.b@msu.ac.th";
				$mail = new PHPMailer();
	...
	กรณีพบปัญหา/ข้อสงสัย ติดต่อที่ graduate@msu.ac.th
	"; //รายละเอียดที่ส่ง
				//$mail->AddAddress($mail_test,$dean_name); //อีเมล์และชื่อผู้รับ
			$mail->AddAddress("kim_4692@hotmail.com",$dean_name); //อีเมล์และชื่อผู้รับ
			$mail->Send();
			echo 1; //เพิ่มข้อมูลสำเร็จ
		} else {
	...
	elseif($Save_admin_Leave=="Save_admin_Leave"){ // Admin Save
		$sql = "Update request_fee_payment set dean_admin_approve_disapprove='$admin_grad_approve_disapprove',dean_admin_node='$argument_a',dean_admin_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
		if($rs_doc){
			echo 1; //เพิ่มข้อมูลสำเร็จ
		} else {
		...
		$sql = "Update request_registration_thesis_is set dean_admin_approve_disapprove='$admin_grad_approve_disapprove',dean_admin_node='$argument_a',dean_admin_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
		if($rs_doc){
			echo 1; //เพิ่มข้อมูลสำเร็จ
		} else {
