<?php 
    ...
    if($finished_assignment==""){
	   $finished_assignment = NULL;
	}
    if($unfinished_assignment==""){
	   $unfinished_assignment = NULL;
	}
    if($unstarting_assignment==""){
	   $unstarting_assignment = NULL;
	}
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_taking_leave rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(1,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_taking_leave (doc_id,semester,academic,because,std_signature,std_date_signature,advisor_chairman ";
		$sql_doc .= ",finished_assigment,unfinished_assignment,unstarting_assignment,advisor_chairman,advisor_chairman_status ";
		$sql_doc .= ",advisor_chairman_signature,advisor_chairman_node,argument,advisor_chairman_date,staff_grad_node ";
		...
		$sql_doc .= "values($max_id,'$semester','$academic','$because','$filename',Now(),'$advisor','$finished_assignment','$unfinished_assignment','$unstarting_assignment') ";
		$sql_doc .= "$advisor_chairman_status,'$semester','$academic','$because','$filename',Now(),'$advisor','$finished_assignment','$unfinished_assignment','$unstarting_assignment' ";
		$sql_doc .= "$max_id,'$semester','$academic','$because','$filename',Now(),'$advisor','$finished_assignment','$unfinished_assignment','$unstarting_assignment', ";
		$sql_doc .= ",$advisor_chairman_status,$advisor_chairman_signature,$advisor_chairman_node,$argument,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL) ";
	    echo $sql_doc;
	...
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ลาพักการเรียน” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต$std_degree_th คณะ$std_faculty_th เมื่อ$date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “ลาพักการเรียน” 
