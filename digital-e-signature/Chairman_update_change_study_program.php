<?php 
	...
	$chairman_board_status = trim($_POST['chairman_board_status']);							
	$argument = trim($_POST['argument']); 	
	$doc_id = trim($_POST['doc_id']);
    $std_id = trim($_REQUEST['std_id']);
	if($chairman_board_status !=''){
		// อาจารย์ได้พิจารณาอกสารคำร้องลาพักการเรียน request_registration_thesis_is
		$sql = "Update request_change_study_program set chairman_board_status='$chairman_board_status',chairman_board_node='$argument',chairman_board_signature='$filename',chairman_board_date=Now() Where doc_id='$doc_id' ";
	    $rs_doc = $mysqli->query( $sql );
		if($rs_doc){
			//=== ส่ง email ข้อมูลอาจารย์
			$sql_mail = "SELECT prefixname,advisorname,advisorsurname,advisor_email FROM request_chairman WHERE advisorcode= '$_SESSION[SES_USER]' "; //session codeoffice 
			...
			$date_now_th = DateThai1(date('Y-m-d H:i:s'));
				...
				$mail->Body = "เรียนประธานกรรมการบริหารหลักสูตร<br>
	เรื่อง คำร้องขอเปลี่ยนแผนการเรียน  <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ขอเปลี่ยนแผนการเรียน” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต$std_degree_th คณะ$std_faculty_th เมื่อ$date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “ขอเปลี่ยนแผนการเรียน” 
