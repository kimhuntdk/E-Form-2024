...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(71,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_document_student_grad (doc_id,government,subject,node,std_signature,std_date_signature) ";
		$sql_doc .= "values($max_id,'$government','$subject','$node','$filename',Now()) ";
		//echo $sql_doc;
	...
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “บันทึกข้อความทั่วไป”  ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “บันทึกข้อความ” 
	...
	"; //รายละเอียดที่ส่ง
				$mail->AddAddress('sarinya.k@msu.ac.th','เจ้าหน้าที่บัณฑิตวิทยาลัย'); //อีเมล์และชื่อผู้รับ
