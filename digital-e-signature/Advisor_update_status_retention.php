<?php 
	...
	if($advisor_chairman_status !=''){
		...
		$sql = "Update request_student_status_retention set advisor_chairman_status='$advisor_chairman_status',advisor_chairman_node='$argument',advisor_chairman_signature='$filename',advisor_chairman_date=Now() Where doc_id='$doc_id' ";
		...
		if($rs_doc){
			...
			$date_now_th = DateThai1(date('Y-m-d H:i:s'));
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “ขอลงทะเบียนรักษาสภาพนิสิต”";  //หัวเรื่อง emal ที่ส่ง
				$mail->Body = "เรียนเจ้าหน้าที่บัณฑิตวิทยาลัย<br>
	เรื่อง ขอลงทะเบียนรักษาสภาพนิสิต <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ขอลงทะเบียนรักษาสภาพนิสิต” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต$std_degree_th คณะ$std_faculty_th เมื่อ$date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “ลาพักการเรียน” 
