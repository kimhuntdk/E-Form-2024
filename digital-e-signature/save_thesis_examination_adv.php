<?php 
	...
	$room_exam = trim($_POST['room_exam']);
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_taking_leave rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(44,'$std_id',Now()) ";
	...
	if($rs_request_doc){
		// เพิ่มข้อมูลเอกสาร
		$sql_doc = "INSERT INTO request_thesis_examination (doc_id,semester,academic,std_code,exam_day,exam_room,std_date_signature,advisor_status,advisor_signature,advisor_date_signature,advisor_code,exam_time) ";
		...
		if($rs_doc){
			...
			if($staff_email!=""){
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “แต่งตั้งคณะกรรมสอบวิทยานิพนธ์”";  //หัวเรื่อง emal ที่ส่ง
				$mail->Body = "เรียนเจ้าหน้าที่ $staff_name_full<br>
	เรื่อง คำร้องแต่งตั้งคณะกรรมสอบวิทยานิพนธ์ <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “แต่งตั้งคณะกรรมสอบวิทยานิพนธ์” ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต$std_degree_th คณะ$std_faculty_th เมื่อ$date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “แต่งตั้งคณะกรรมสอบวิทยานิพนธ์” 
