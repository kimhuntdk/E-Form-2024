<?php 
	...
	if($dean_admin_approve_disapprove !=''){
		...
		$sql = "Update request_taking_leave set dean_admin_approve_disapprove='$dean_admin_approve_disapprove',dean_admin_node='$argument',dean_admin_signature='$filename',dean_admin_date=Now() Where doc_id='$doc_id' ";
		...
		if($rs_doc){
			...
			$date_now_th = DateThai1(date('Y-m-d H:i:s'));
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “คำร้องลาพักการเรียน”";  //หัวเรื่อง emal ที่ส่ง
