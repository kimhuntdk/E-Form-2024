...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(3,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_registration_thesis_is (doc_id,status_thesis_is,subject_code,credits,group_id,classid,semester,academic,because,std_signature,std_date_signature,advisor_chairman ,argument,advisor_chairman_status,advisor_chairman_signature,advisor_chairman_date,advisor_chairman_node,staff_grad_node ";
        $sql_doc .= ",staff_grad_date,staff_grad_approve_disapprove,dean_admin,dean_admin_approve_disapprove,dean_admin_signature ";
         $sql_doc .= ",dean_admin_node,dean_admin_date,registration_division_status,registration_division_node,payment)";
		$sql_doc .= "values($max_id,'$type_thesis','$subject_code','$credits','$group','$classid','$semester','$academic','$because','$filename',Now(),'$advisor','NULL',0,'NULL',Now(),'NULL','NULL',Now(),0,0,0,'NULL','NULL',Now(),0,'NULL',0) ";
        //echo $sql_doc;
		...
		if($rs_doc){
			...
			$name_full = $row_std['std_fname_en']." ".$row_std['std_lname_en'];
			if($email!=""){
				$mail = new PHPMailer();
				$mail->isSMTP();
				$mail->SMTPAuth = true;
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “ขอลงทะเบียน Thesis/IS”";  //หัวเรื่อง emal ที่ส่ง
				$mail->Body = "เรียนอาจารย์ที่ปรึกษา $advisor_name_full<br>
	เรื่อง คำร้องขอลงทะเบียน Thesis/IS <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ลงทะเบียน Thesis/IS” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $name_full  รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “ขอลงทะเบียน Thesis/IS” 
