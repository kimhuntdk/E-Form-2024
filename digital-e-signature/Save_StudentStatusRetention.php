...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_student_status_retention (doc_id,semester,academic,because,std_signature,std_date_signature,advisor_chairman) ";
		$sql_doc .= "values($max_id,'$semester','$academic','$because','$filename',Now(),'$advisor') ";
	...
	เรื่อง คำร้องขอลงทะเบียนรักษาสภาพนิสิต <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ขอลงทะเบียนรักษาสภาพนิสิต ” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “ขอลงทะเบียนรักษาสภาพนิสิต ” 
