<?php 
	...
	$title_th = $mysqli->real_escape_string($_POST['title_th']);		
	$title_en = $mysqli->real_escape_string($_POST['title_en']);										
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(40,'$std_id',Now()) ";
	...
	//== การ เพิ่ม email ในการส่งคำร้อง
  	$sql_insert_mail = "INSERT INTO request_send_email (email_name,doc_id,doc_std_id,doc_date)values('$email','$max_id','$std_id',Now()) ";
	$mysqli->query( $sql_insert_mail );
	if($rs_request_doc){
		// เพิ่มข้อมูลเอกสาร
		$sql_doc = "INSERT INTO request_appointment_advisor (doc_id,semester,academic,title_th,title_en,std_code,advisor_code,advisor_status,advisor_signature,advisor_date_signature) ";
		$sql_doc .= "values($max_id,'$semester','$academic','$title_th','$title_en','$std_id','$_SESSION[SES_USER]',1,'$filename',Now()) ";
	  //echo $sql_doc;
...
/*			$sql_mail = "SELECT prefixname,advisorname,advisorsurname,advisor_email FROM request_advisor WHERE advisorcode=".$_SESSION["SES_USER"];
			...
			if($staff_email!=""){
				...
				$mail->Subject = "[e-Form Graduate MSU] อาจารย์ได้ส่งคำร้อง “แต่งตั้งชื่อเรื่องวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ”";  //หัวเรื่อง emal ที่ส่ง
				$mail->Body = "เรียนเจ้าที่ $staff_name_full<br>
	เรื่อง คำร้องแต่งตั้งชื่อเรื่องวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “แต่งตั้งชื่อเรื่องวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ”  ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  
    <br><br>
