<?php 
	...
	$curriculum_completed = trim($_POST['curriculum_completed']);
	$credits = trim($_POST['credits']);		
	$ce_qe = trim($_POST['ce_qe']);
	$thesis_approved = trim($_POST['thesis_approved']);		
	$thesis_completed = trim($_POST['thesis_completed']);		
	$advisor = trim($_POST['advisor']);	
	$email = trim($_POST['email']);	
	//$dateto_ = DATE_FORMAT($dateto,  '%Y-%m-%d' );
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(12,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_return_work (doc_id,semester,academic,date_return_work,curriculum_completed,study_semester,credits,ce_qe,thesis_approved,thesis_completed,std_signature,std_date_signature,advisor_chairman) ";
		$sql_doc .= "values($max_id,'$semester','$academic','$dateto','$curriculum_completed','$study_semester','$credits','$thesis_approved','$thesis_completed','$ce_qe','$filename',Now(),'$advisor') ";
	    //echo $sql_doc;
		...
		if($rs_doc){
			...
			if($email!=""){
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “ขอกลับเข้ารับราชการ”";  //หัวเรื่อง emal ที่ส่ง
				$mail->Body = "เรียนอาจารย์ที่ปรึกษา $advisor_name_full<br>
	เรื่อง คำร้องขอกลับเข้ารับราชการ <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ขอกลับเข้ารับราชการ” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “ขอกลับเข้ารับราชการ” 
