<?php 
	...
	$status_master_phd = trim($_POST['status_master_phd']);	
	$form_program_plan_master = trim($_POST['form_program_plan_master']);
	$to_program_plan_master = trim($_POST['to_program_plan_master']);
	$form_program_plan_phd = trim($_POST['form_program_plan_phd']);
	$to_program_plan_phd = trim($_POST['to_program_plan_phd']);
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(9,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_change_study_program (doc_id,semester,academic,status_master_phd,form_program_plan_master,to_program_plan_master,form_program_plan_phd,to_program_plan_phd,std_signature,std_date_signature,advisor_chairman,chairman_board) ";
		$sql_doc .= "values($max_id,'$semester','$academic','$status_master_phd','$form_program_plan_master','$to_program_plan_master','$form_program_plan_phd','$to_program_plan_phd','$filename',Now(),'$advisor','$chairman') ";
	  //echo $sql_doc;
		...
		if($rs_doc){
			...
			if($email!=""){
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “ขอเปลี่ยนแผนการเรียน”";  //หัวเรื่อง emal ที่ส่ง
				$mail->Body = "เรียนอาจารย์ที่ปรึกษา $advisor_name_full<br>
	เรื่อง คำร้องขอเปลี่ยนแผนการเรียน <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ขอเปลี่ยนแผนการเรียน” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “คำร้องขอเปลี่ยนแผนการเรียน” 
