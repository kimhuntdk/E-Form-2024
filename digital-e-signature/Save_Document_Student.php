<?php 
	...
	$advisor_chairman_dean_status = trim($_POST['advisor_chairman_dean_status']);	
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(70,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_document_student (doc_id,government,subject,node,std_signature,std_date_signature,advisor_chairman,advisor_chairman_dean_status) ";
		$sql_doc .= "values($max_id,'$government','$subject','$node','$filename',Now(),'$advisor','$advisor_chairman_dean_status') ";
		//echo $sql_doc;
		...
		if($rs_doc){
			...
			if($email!=""){
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “บันทึกข้อความ”";  //หัวเรื่อง emal ที่ส่ง 
				$mail->Body = "เรียนอาจารย์ที่ปรึกษา $advisor_name_full<br>
	เรื่อง  $subject<br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “บันทึกข้อความ”  ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “บันทึกข้อความ” 
	...
	"; //รายละเอียดที่ส่ง
				$mail->AddAddress($advisor,$advisor_name_full); //อีเมล์และชื่อผู้รับ
