<?php 
	...
	$semester_enroll = trim($_POST['semester_enroll']);		
	$academic_enroll = trim($_POST['academic_enroll']);
	$semester = trim($_POST['semester']);		
	$academic = trim($_POST['academic']);			
	$subjects_not_ompleted = trim($_POST['subjects_not_ompleted']);
	$failed_exam = trim($_POST['failed_exam']);
	$thesis_not_completed = trim($_POST['thesis_not_completed']);
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(11,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_extension_period_study (doc_id,semester_enroll,semester,academic_enroll,academic,subjects_not_ompleted,failed_exam,thesis_not_completed,std_signature,std_date_signature,advisor_chairman) ";
		$sql_doc .= "values($max_id,'$semester_enroll','$semester','$academic_enroll','$academic','$subjects_not_ompleted','$failed_exam','$thesis_not_completed','$filename',Now(),'$advisor') ";
		...
		if($rs_doc){
			...
			if($email!=""){
				...
				$mail->Subject = "[e-Form Graduate MSU] นิสิตได้ส่งคำร้อง “ชำระค่าธรรมเนียมการศึกษา”";  //หัวเรื่อง emal ที่ส่ง
				$mail->Body = "เรียนอาจารย์ที่ปรึกษา $advisor_name_full<br>
	เรื่อง คำร้องชำระค่าธรรมเนียมการศึกษา <br><br>
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ชำระค่าธรรมเนียมการศึกษา” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “ชำระค่าธรรมเนียมการศึกษา” 
