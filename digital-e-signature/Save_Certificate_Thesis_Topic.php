...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	//รหัสประเภท 97 คือ หนังสือรับรองการอนุมัติหัวข้อและโครงร่างวิทยานิพนธ์
	 $sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(97,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_certificate_topic_thesis (doc_id,academic,subject,std_email,std_signature,std_date_signature) ";
		$sql_doc .= "values($max_id,'$academic','$title_th','$email','$filename',Now()) ";
	    //echo $sql_doc;
	...
	เรื่อง หนังสือรับรองการอนุมัติหัวข้อและโครงร่างวิทยานิพนธ์ <br><br>
		ด้วยข้าพเจ้า $name 
		 
	ผ่านระบบได้ที่นี้ คลิก https://grad.msu.ac.th/electronic/Doc-Certificate-Topic-Thesis.php?id=$id_  
	...
	"; //รายละเอียดที่ส่ง
				...
				//$mail->AddAddress('watthanachai.s@msu.ac.th','เจ้าหน้าที่บัณฑิตวิทยาลัย'); //อีเมล์และชื่อผู้รับ
