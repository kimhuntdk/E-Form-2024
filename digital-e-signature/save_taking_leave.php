<?php 
	...
	if(!isset($_POST['finished_a'])){
        $finished_a = 0; 
	}
	if(!isset($_POST['unfinished_a'])){
		  $unfinished_a = 0; 
	}
	if(!isset($_POST['unstarting_a'])){
		  $unstarting_a = 0; 
	}
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_taking_leave rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	if($rs_request_doc){
		...
		$sql_doc .= ",finished_assigment,unfinished_assignment,unstarting_assignment,advisor_chairman_status ";
		...
		$sql_doc .= ", '0', 'NULL' , 'NULL', 'NULL', Now(), 'NULL', '0', Now(), '0', '0', 'NULL', 'NULL',Now(), '0', 'NULL', '0')" ;
	     //echo $sql_doc;
