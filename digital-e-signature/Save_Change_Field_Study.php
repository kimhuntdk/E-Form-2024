<?php 
	...
	$major_old = trim($_POST['major_old']);		
	$major_new = trim($_POST['major_new']);										
...
/*	$sql_chk = "SELECT rd.doc_type,rd.doc_std_id,rgt.subject_code,rgt.semester,rgt.academic,rgt.advisor_chairman_status FROM request_doc rd left join request_registration_thesis_is rgt on rd.doc_id = rgt.doc_id WHERE rd.doc_std_id=".$_SESSION["SES_STDCODE"];
	...
	$sql_request_doc = "INSERT INTO request_doc (doc_type,doc_std_id,doc_date)values(8,'$_SESSION[SES_STDCODE]',Now()) ";
	...
	if($rs_request_doc){
		...
		$sql_doc = "INSERT INTO request_change_field_study (doc_id,semester,academic,major_old,major_new,std_signature,std_date_signature,advisor_chairman,chairman_board) ";
		$sql_doc .= "values($max_id,'$semester','$academic','$major_old','$major_new','$filename',Now(),'$advisor','$chairman') ";
	  //echo $sql_doc;
	...
	ระบบ e-Form Graduate MSU ได้ส่งคำร้อง “ขอเปลี่ยนสาขาวิชา” ภาคเรียนที่ $semester/$academic ของนิสิตชื่อ $_SESSION[SES_STDNAME_FULL_TH] รหัส $std_id_std  นิสิต $std_degree_th คณะ$std_faculty_th เมื่อ $date_now_th  ได้โปรดรพิจารณาลงนามคำร้อง “คำร้องขอเปลี่ยนสาขาวิชา” 
