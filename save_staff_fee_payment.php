...
$Save_Staff_fee_payment = trim( $_POST[ 'Save_Staff_fee_payment' ] );
...
if ( $Save_Staff_fee_payment == "Save_Staff_fee_payment" ) { // Staff Save
	...
	if ( $staff_grad_approve_disapprove == 2 ) { //กรณีใช้แบบฟอร์มผิดประเภท
		 ...
		 $sql = "Update request_fee_payment set staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_node='$argument_a',staff_grad_date=Now() Where doc_id='$doc_id' ";
		$rs_doc = $mysqli->query( $sql );
		if ( $rs_doc ) {
	...
	if ( $staff_grad_approve_disapprove == 1 ) { //กรณีผ่านเรื่องให้ผู้บริหารพิจารณา
		$sql = "Update request_fee_payment set dean_admin='$dean_admin',staff_grad_approve_disapprove='$staff_grad_approve_disapprove',staff_grad_node='$argument_a',staff_grad_date=Now() Where doc_id='$doc_id' ";
		$rs_doc = $mysqli->query( $sql );
		if ( $rs_doc ) {
			$mail_test = "jakkrid.b@msu.ac.th";
			$mail = new PHPMailer();
